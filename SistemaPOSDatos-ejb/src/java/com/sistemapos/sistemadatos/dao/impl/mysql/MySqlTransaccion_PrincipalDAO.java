/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */

package com.sistemapos.sistemadatos.dao.impl.mysql;

import com.sistemapos.sistemadatos.dao.interfaces.Transaccion_PrincipalDAOInterface;
import com.sistemapos.sistemadatos.dto.Transaccion_PrincipalDTO;
import java.util.List;

/**
 *
 * @author cmeza
 */
public class MySqlTransaccion_PrincipalDAO implements Transaccion_PrincipalDAOInterface{
    
     @Override   
    public int ingresar(Transaccion_PrincipalDTO u) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public int eliminar(Transaccion_PrincipalDTO u) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public int actualizar(Transaccion_PrincipalDTO u) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public List<Transaccion_PrincipalDTO> obtenerTodo() {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
}
